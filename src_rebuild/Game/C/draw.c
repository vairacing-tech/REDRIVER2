...
void DrawAllTheCars(int view)
{
	...
	if (num_cars_to_draw != 0)
	{
		...
		for (i = 0; i < num_cars_to_draw; i++)
		{
			...
			if ((intptr_t)(current->primtab + (-3000 - (intptr_t)(current->primptr - PRIMTAB_SIZE))) < 5800)
				return;

			// make cars look uglier
			if ((intptr_t)(current->primtab + (-3000 - (intptr_t)(current->primptr - PRIMTAB_SIZE)) - spacefree) < 5800)
